...
CREATE TABLE [chart_lock_events] (
  ...
  [unlock_reason] NVARCHAR(MAX),
  [incident_id] BIGINT NOT NULL,
  [unlocked_by] BIGINT,
  [event_time_lock] DATETIMEOFFSET(0) NOT NULL DEFAULT (SYSDATETIMEOFFSET()),
  [event_time_unlock] DATETIMEOFFSET(0) NOT NULL DEFAULT (SYSDATETIMEOFFSET())
)
GO
...
CREATE TABLE [durable_medical_equipment] (
    ...
    [status] VARCHAR(30) NOT NULL DEFAULT ('AVAILABLE')
        CHECK ([status] IN ('AVAILABLE', 'IN_SERVICE', 'UNDER_MAINTENANCE', 'RETIRED')),
    [facility_id] UNIQUEIDENTIFIER NOT NULL,
    [assigned_to_user] UNIQUEIDENTIFIER NULL,
    [assigned_to_resident] UNIQUEIDENTIFIER NULL,
    [unit_value] DECIMAL(18,2) NOT NULL DEFAULT (0),
    [is_deleted] BIT NOT NULL DEFAULT (0),
    [created_at] DATETIMEOFFSET(0) NOT NULL DEFAULT (SYSDATETIMEOFFSET()),
    [updated_at] DATETIMEOFFSET(0) NOT NULL DEFAULT (SYSDATETIMEOFFSET()),

    CONSTRAINT [FK_dme_category]
        FOREIGN KEY ([category_id]) REFERENCES [inventory_categories]([id]),

    CONSTRAINT [FK_dme_facility]
        FOREIGN KEY ([facility_id]) REFERENCES [facilities]([id]),

    CONSTRAINT [FK_dme_user]
        FOREIGN KEY ([assigned_to_user]) REFERENCES [users]([id]),

    CONSTRAINT [FK_dme_resident]
        FOREIGN KEY ([assigned_to_resident]) REFERENCES [residents]([id])
)
GO
CREATE INDEX [idx_dme_facility_lookup]
ON [durable_medical_equipment]([facility_id], [is_deleted])
GO

CREATE INDEX [idx_dme_status_filter]
ON [durable_medical_equipment]([status])
WHERE [is_deleted] = 0
GO

CREATE INDEX [idx_dme_item_name]
ON [durable_medical_equipment]([item_name])
GO
...
CREATE TABLE [consumable_supplies] (
    ...
    [status] VARCHAR(20) NOT NULL DEFAULT ('OK')
        CHECK ([status] IN ('OK', 'LOW_STOCK', 'OUT_OF_STOCK')),
    [is_deleted] BIT NOT NULL DEFAULT (0),
    [created_at] DATETIMEOFFSET(0) NOT NULL DEFAULT (SYSDATETIMEOFFSET()),
    [updated_at] DATETIMEOFFSET(0) NOT NULL DEFAULT (SYSDATETIMEOFFSET()),
    ...
    CONSTRAINT [FK_supplies_facility]
        FOREIGN KEY ([facility_id]) REFERENCES [facilities]([id])
)
GO

CREATE INDEX [idx_supplies_facility_lookup]
ON [consumable_supplies]([facility_id], [is_deleted])
GO

CREATE INDEX [idx_supplies_status_filter]
ON [consumable_supplies]([status])
WHERE [is_deleted] = 0
GO
...
ALTER TABLE [chart_lock_events] ADD FOREIGN KEY ([incident_id]) REFERENCES [incidents] ([id])
GO

ALTER TABLE [chart_lock_events] ADD FOREIGN KEY ([unlocked_by]) REFERENCES [users] ([id])
GO
