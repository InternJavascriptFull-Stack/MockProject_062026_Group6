...
const otpStore = new Map<
  string,
  { code: string; expiresAt: Date; attempts: number; resends: number }
>();
...
export class AuthService {
  ...
  async login(dto: LoginDto) {
    ...
    const user = await this.prisma.user.findFirst({
      where: isEmail
        ...
        : { phoneNumber: identifier, isDeleted: false },
    });

    // Timing-safe: run bcrypt even when user not found
    const hashToCompare = user?.passwordHash?.trim()
      ? user.passwordHash
      : "$2b$12$AAAAAAAAAAAAAAAAAAAAAA.AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
    ...
    if (isUnactivated(user.status)) {
      throw new BadRequestException(
        "Account not activated, check invite link.",
      );
    }
  ...
  async verifyOtp(dto: VerifyOtpDto) {
    ...
    if (!stored) {
      throw new BadRequestException(
        "OTP session not found or expired. Please log in again.",
      );
    }
  ...
  async resendOtp(dto: ResendOtpDto) {
    ...
    if (!stored) {
      throw new BadRequestException(
        "No active OTP session. Please log in again.",
      );
    }
    ...
    if (new Date() > stored.expiresAt) {
      otpStore.delete(email);
      throw new BadRequestException(
        "OTP session has expired. Please log in again.",
      );
    }
