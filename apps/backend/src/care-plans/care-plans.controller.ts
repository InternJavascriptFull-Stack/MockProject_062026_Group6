...
export class CarePlansController {
  ...
  @Post(':id/check-loc-gate')
  @ApiOperation({ summary: 'Kiểm tra cảnh báo LOC Gate' })
  async checkLocGate(@Param('id') id: string, @Body() body: any) {
    // any is fine here if they send empty body or we can omit forbidNonWhitelisted for this route if needed, but let's use @Allow
    return this.carePlansService.checkLocGate(id, body);
  }
